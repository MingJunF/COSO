...
class MIMIC3RealDatasetCollection(RealDatasetCollection):
    ...
    def __init__(self,
        ...
        #compute COSO
        # 假设 treatments, outcomes, coso_vitals 是初始的DataFrame，且均有MultiIndex (patient_id, timestep)
        user_sizes = treatments.index.get_level_values(0).value_counts().sort_index()

        # Step 1: 数据预处理
        # 对每个DataFrame进行unstack - stack操作，保证数据的完整性
